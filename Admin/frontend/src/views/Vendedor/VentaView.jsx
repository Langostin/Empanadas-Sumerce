...
export default function VentaView() {
  ...
  const handeProcesarVenta = async () => {
    ...
    if (metodoPago === 1 && efectivoRecibido < totalCarrito) {
      setMensaje("El cobro en efectivo es menor al total de la cuenta.");
      setTipoMensaje("error");
      return;
    }
    if (requiereFactura && (!datosFiscales.whatsapp || !datosFiscales.rfc)) {
      setMensaje("Debe proporcionar teléfono y RFC para la factura.");
      setTipoMensaje("error");
      return;
    }
    ...
    try {
      const payload = {
        ...
        efectivo_recibido: parseFloat(efectivoRecibido) || 0,
        datos_fiscales: requiereFactura ? datosFiscales : null
      };

      const token = localStorage.getItem("accessToken");
      const res = await fetch("/api/vendedor/crear_venta", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
      ...
      if (res.ok) {
        setMensaje(`✅ ¡Venta cerrada correctamente! Folio: ${data.folio}`);
        ...
        setMensaje(`❌ Error: ${data.error || "No se pudo procesar la venta"}`);
        setTipoMensaje("error");
      }

    } catch (e) {
      console.error("Error en handeProcesarVenta:", e);
      setMensaje(`❌ Error procesando pago: ${e.message}`);
      setTipoMensaje("error");
    } finally {
      setProcesando(false);
    }
        ...
        <DialogContent dividers>
          ...
          {mensaje && (
            <Box
              ...
              sx={{
                borderRadius: 2,
                background: tipoMensaje === "success" ? alpha("#18A558", 0.1) : alpha("#D32F2F", 0.1),
                border: `1px solid ${tipoMensaje === "success" ? "rgba(24,165,88,0.3)" : "rgba(211,47,47,0.3)"}`,
                color: tipoMensaje === "success" ? "#18A558" : "#D32F2F"
              }}
            >
